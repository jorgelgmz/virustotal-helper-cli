...
const getSentinelOne = async (api) => {
  ...
  await fs.appendFile(path.join(process.cwd(), 'output.csv'), `Name,Category,Engine Name,Engine Version,Result,Method,Engine Update\n`, 'utf-8');
    ...
    try {
      ...
      if (await scanners.SentinelOne) {
        await fs.appendFile(
          ...
          `${response.data.data.attributes.names.toString().replaceAll(',', ';')},${scanners.SentinelOne.category},${
